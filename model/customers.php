...
class Customers
{
    ...
    {
        $conn = dbCon();
        $val = mysqli_query($conn, "SELECT * FROM tbl_customers LIMIT 1");
        if ($val !== FALSE) {
            return (bool) true;
        } else {
            return $this->create();
    ...
    function insert($keys)
    {
        ...
        if ($this->isTable() === true) {
            ...
            $params = implode("`, `", $keys);
            $sql = "INSERT INTO tbl_customers (`" . $params . "`) VALUES ('" . $importData . "')";

            $q = mysqli_query($conn, $sql);
            $id = mysqli_insert_id($conn);
            if ($q === true) {
                $sql = "UPDATE tbl_customers SET `user_token`='" . $this->userToken($id) . "' WHERE id=" . $id;
            ...
            var_dump("Failed in Creating tbl_customers Table");
            die;
        }
    }
    ...
    function searchClient()
    {
        ...
        if ($this->isTable() === true) {
            ...
            $query = "SELECT * FROM tbl_customers $where";
    ...
    function update($id = NULL)
    {
        ...
        $sql = "UPDATE tbl_customers SET " . $this->data . " WHERE id=" . $id;
    ...
    function generatePDF($id)
    {
        ...
        $data = $this->data;
        $output = "../uploads/c742a402367f2030636af25ce0c9713ehemoglobin" . $id . ".pdf";
        ...
        $awsPdfFile = "8513da060dea559ed3dff467ecb2651ehemoglobin" . $id . ".pdf";

        $url = $this->uploadReportToAwsS3($output, $awsPdfFile);

        $conn = dbCon();
        $sql = "UPDATE tbl_customers SET pdfLink='" . $url . "' WHERE id=" . $id;
        $q = mysqli_query($conn, $sql);
    ...
    function sendEmail($data, $URL)
    {
        ...
        $subject = "Hemoglobin Screening [" . $data['firstname'] . " " . $data['lastname'] . "]";
